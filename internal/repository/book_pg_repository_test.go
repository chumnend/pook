...
func TestBookRepository_FindAll(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		...
		repo := NewBookPostgresRepository(gdb)
...
func TestBookRepository_FindAllByUserID(t *testing.T) {
	db, mock, err := sqlmock.New()
	if err != nil {
		t.Fatal("an error occured when opening stub database", err)
	}
...
func TestRepo_FindByID(t *testing.T) {
	...
	t.Run("fail", func(t *testing.T) {
		...
		// check
		mock.ExpectationsWereMet()
		assert.Equal(t, entity.Book{}.ID, book.ID)
		assert.Equal(t, entity.Book{}.Title, book.Title)
		assert.Equal(t, entity.Book{}.UserID, book.UserID)
		assert.Error(t, err)
	})
}
